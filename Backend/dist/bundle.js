...
/***/505:
/***/function(e,t,r){var a=this&&this.__importDefault||function(e){return e&&e.__esModule?e:{default:e}};Object.defineProperty(t,"__esModule",{value:!0}),t.server=void 0;const i=a(r(860)),n=r(986),s=r(231),o=r(582),d=s.readFileSync("./certificates/key.pem"),c=s.readFileSync("./certificates/cert.pem"),u=r(14),l=a(r(993)),m=a(r(617)),p=(r(685),a(r(811))),h=a(r(79)),f=a(r(562)),g=a(r(11)),y=a(r(996)),w=a(r(653)),v=a(r(109)),b=a(r(952)),E=(0,i.default)(),S=process.env.PORT||443;
/* Routes */
E.use("*",o()),E.use((function(e,t,r){t.header("Access-Control-Allow-Origin","*"),t.header("Access-Control-Allow-Headers","Origin, X-Requested-With, Content-Type, Accept"),t.header("Access-Control-Expose-Headers","Authorization"),r()})),t.server=m.default.createServer({key:d,cert:c},E);r(69)(t.server,{cors:{origin:"http://localhost:3000",methods:["GET","POST"]}}).on("connection",(e=>{console.log("socket connected"),e.on("disconnect",(()=>{console.log("socket disconnected")})),e.on("join_room",(({roomId:t})=>{e.join(t)})),e.on("send_message",(({user:t,message:r,roomId:a})=>{console.log(`(${a})> ${t}: ${r}`),e.to(a).emit("receive_message",{user:t,message:r})}))})),E.use(n.json()),E.use(i.default.urlencoded({extended:!0})),E.use(i.default.static(__dirname)),
// Health check endpoint
E.get("/health",((e,t)=>{t.status(200).send("Health Check: OK")})),E.use("/api/users",h.default),E.use("/api",f.default),E.use("/api/login",g.default),E.use("/api/events",y.default),E.use("/api/comments",v.default),E.use("/api/chat",b.default),(0,p.default)(E,+S),E.use(((e,t,r)=>{t.status(404).json("Not Found")})),(0,u.connect)().then((async()=>{
...
/***/659:
/***/(e,t,r)=>{Object.defineProperty(t,"__esModule",{value:!0}),t.ChatService=void 0;const a=r(924),i=r(95);t.ChatService=class{async getChat(e){if(!e)throw new Error(`Chat ID: ${e} is invalid.`);const t=await a.Chat.findById(e).exec();if(!t)throw new Error("Chat not found");return{id:t.id,event:t.event.toString(),messages:await Promise.all(t.messages.map((async e=>{const t=await i.User.findById(e.user);return{user:e.user.toString(),username:`${t.name.first} ${t.name.last}`,message:e.message.toString()}})))}}async sendMessage(e,t,r){if(!e)throw new Error(`Chat ID: ${e} is invalid.`);const n=await a.Chat.findById(e).exec();if(!n)throw new Error("Chat not found");const s=await i.User.findById(t).exec();if(!s)throw new Error("User not found");const o=await a.Event.findById(n.event).exec();if(!o)throw new Error("Event not found");if(!o.participants.includes(s._id))throw new Error("User is not participating in the event");n.messages.push({user:s._id,message:r});const d=await n.save();return{id:d.id,event:d.event.toString(),messages:await Promise.all(d.messages.map((async e=>{const t=await i.User.findById(e.user);return{user:e.user.toString(),username:`${t.name.first} ${t.name.last}`,message:e.message.toString()}})))}}}}
/***/,
/***/282:
