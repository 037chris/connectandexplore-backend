...
/* Routes */
E.use("*",o()),E.use((function(e,t,r){t.header("Access-Control-Allow-Origin","*"),t.header("Access-Control-Allow-Headers","Origin, X-Requested-With, Content-Type, Accept"),t.header("Access-Control-Expose-Headers","Authorization"),r()})),t.server=m.default.createServer({key:d,cert:c},E);const I=r(69)(t.server,{cors:{origin:"http://localhost:3000",methods:["GET","POST"]}});I.on("connection",(e=>{console.log("socket connected"),e.on("disconnect",(()=>{console.log("socket disconnected")})),e.on("join chat",(async t=>{const{chatID:r,username:a}=t;e.join(r),e.username=a,console.log(`(${r}): ${a} joined`)})),e.on("new message",(t=>{const{chatID:r,message:a}=t;console.log(`(${r}): ${e.username}> ${a}`),I.to(r).emit("new message",{username:e.username,message:a})}))})),E.use(n.json()),E.use(i.default.urlencoded({extended:!0})),E.use(i.default.static(__dirname)),
