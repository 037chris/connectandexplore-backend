...
/* Routes */
j.use("*",o()),j.use((function(e,t,r){t.header("Access-Control-Allow-Origin","*"),t.header("Access-Control-Allow-Headers","Origin, X-Requested-With, Content-Type, Accept"),t.header("Access-Control-Expose-Headers","Authorization"),r()})),t.server="443"===I?m.createServer(j):p.default.createServer({key:d,cert:c},j);r(69)(t.server,{cors:{
...
hashtags:["Tech","Livepräsentation"],thumbnail:"/FOOTER_LOGO.png",participants:[g.id,f.id,y.id,p.id,m.id,h.id,v.id,y.id,w.id]};await i.default.createEvent(E,p.id),await i.default.createEvent(S,p.id),await i.default.createEvent(j,p.id),await i.default.createEvent(O,p.id),await i.default.createEvent(k,p.id),await i.default.createEvent(q,m.id),await i.default.createEvent(I,m.id),await i.default.createEvent(x,m.id),await i.default.createEvent(C,h.id),await i.default.createEvent(_,g.id),
